...
func InitLogger() {
	var (
		filename  = config.GetConf().Logs.FileName
		maxSize   = config.GetConf().Logs.MaxSize
		maxBackup = config.GetConf().Logs.MaxBackup
		maxAge    = config.GetConf().Logs.MaxAge
		compress  = config.GetConf().Logs.Compress
		logType   = config.GetConf().Logs.Type
		level     = config.GetConf().Logs.Level
	)
	// 获取日志写入介质
...
func getLogWriter(filename string, maxSize, maxBackup, maxAge int, compress bool, logType string) zapcore.WriteSyncer {
	// 如果配置了按照日期记录日志文件
	if logType == "daily" {
		logName := time.Now().Format("server-2006-01-02.log")
